...
public:
    ...
    void Subdivide(bool conservative=true)
    {
        ...
        float fx=0, fy=0;
        if ( subdiv < 0 ) {
            unsigned int rx = width % (1<<-subdiv);
            if ( rx > 0 ) endX++;
            if ( rx > (1<<-s) ) {
                endX++;
                fx = float(1<<-s) / float(rx);
            }
            unsigned int ry = height % (1<<-subdiv);
            if ( ry > 0 ) endY++;
            if ( ry > (1<<-s) ) {
                endY++;
...
private:
    ...
    public:
        ...
        void Clear(unsigned int i) { validity[i>>3] &= ~((char)(1 << (i&7))); }
        void Set  (unsigned int i) { validity[i>>3] |=   (char)(1 << (i&7));  }
