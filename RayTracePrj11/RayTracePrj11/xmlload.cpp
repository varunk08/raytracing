...
int LoadScene(const char *filename)
{
    ...
    while ( camChild ) {
        ...
        else if ( COMPARE( camChild->Value(), "shutter"   ) ) ReadFloat (camChild, camera.shutter);
...
void LoadTransform( Transformation *trans, TiXmlElement *element, int level )
{
    for ( TiXmlElement *child = element->FirstChildElement(); child!=NULL; child = child->NextSiblingElement() ) {
        ...
        } else if(COMPARE(child->Value(), "motion")){
            trans->SetMotion(true);
            for(TiXmlElement *instance = child->FirstChildElement(); instance!=NULL; instance = instance->NextSiblingElement()){
                for(TiXmlElement *transform = instance->FirstChildElement(); transform!=NULL; transform = transform->NextSiblingElement()){
                    if(COMPARE(transform->Value(), "translate")){
                        Point3 t(0,0,0);
                        ReadVector(transform, t);
                        trans->SetAdditionalPosition(t);
                        PrintIndent(level);
                        printf(" ....motion %f %f %f\n",t.x,t.y,t.z);
                    }
                }
