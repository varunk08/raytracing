...
public:
    ...
    void    ComputeZBufferImage()
    {
        ...
        float zmin=BIGFLOAT, zmax=-BIGFLOAT;
        ...
        for ( int i=0; i<size; i++ ) {
            if ( zbuffer[i] == BIGFLOAT || zbuffer[i] == 0){// zbuffer8[i] = 0;
                zbuffer8[i] = 0;
            }
            else {
    ...
    bool SaveImage(const char *filename) const { SavePPM(filename,&img[0].r,3,true); }
    bool SaveZImage(const char *filename) const { SavePPM(filename,zbuffer8,1, true); }
private:
