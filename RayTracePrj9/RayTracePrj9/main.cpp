...
#define MIN_N_SAMPLES 32
#define MAX_N_SAMPLES 128
#define H_BASE_1 2
#define H_BASE_2 3
#define SHADE_THRESHOLD 1e-3f
#define OBJECT_N_TIME_STEPS 3
...
void doRender(void* arg){
    ...
    if(rootNode.GetNumChild()>0){
        for(int i=0; i< MIN_N_SAMPLES;i++){
            ...
            hitInfo.timeInstance = (rand()/(float) RAND_MAX) * camera.shutter;

            if(RayTrace_2(r, hitInfo)) {
                pixelHit=true;
                shade = hitInfo.node->GetMaterial()->Shade(r, hitInfo, lights, BOUNCE_COUNT);
...
Color MtlBlinn::Shade(const Cone &ray, const HitInfo &hInfo, const LightList &lights, int bounceCount) const{
    ...
    if(refraction.GetColor().r>0 && bounceCount>0){
        ...
        HitInfo temp;
        temp.Init();
        temp.timeInstance = (rand()/(float) RAND_MAX) * camera.shutter;
        Point3 N = hInfo.N;
        ...
        if(bounceCount >0){
            ...
            HitInfo temp1;
            temp1.Init();
            temp1.timeInstance = (rand()/(float) RAND_MAX) * camera.shutter;
            if(rootNode.GetNumChild()>0){
...
bool TraceNode(const Cone &r, HitInfo &hInfo,const Node &node)
{
    ...
    Cone ray = r;
    if( node.IsInMotion()){
        ray = node.ToNodeCoords(r, hInfo.timeInstance);
    }
...
{
    const char* filename = "/Users/varunk/Desktop/RayTracerProj1/RayTracePrj9/RayTracePrj9/scene.xml";
    LoadScene(filename);
